...
public class PearlXPListener implements org.bukkit.event.Listener {
	...
	public void onPlayerInteract(org.bukkit.event.player.PlayerInteractEvent event){
		...
		if (item != null && item.getTypeId() == instance.getConfig().getInt("itemid")) {
			Player ply = event.getPlayer();
			String itemname = "pearl";
			if (item.getTypeId() != 368) {itemname = "item";}
			...
			if (item.containsEnchantment(org.bukkit.enchantments.Enchantment.WATER_WORKER) && (event.getAction() == org.bukkit.event.block.Action.RIGHT_CLICK_BLOCK || event.getAction() == org.bukkit.event.block.Action.RIGHT_CLICK_AIR)) {
				...
				ply.sendMessage("This "+itemname+" is imbued with "+xpeditor.xpLookup[level]+"xp (level 0->"+level+")!");
				event.setCancelled(true); return;
			}
			else if (event.getAction() == org.bukkit.event.block.Action.RIGHT_CLICK_BLOCK && item.getTypeId() == 368) {ply.sendMessage("Throwing a pearl at your feet would hurt!"); event.setCancelled(true); return;}
				...
				if (item.getAmount() > 1) {
					ply.sendMessage("To store experience in the "+itemname+", please unstack them!");
					return;
				}
				XPEditor xpeditor = new XPEditor(ply);
				...
				if (item.containsEnchantment(org.bukkit.enchantments.Enchantment.WATER_WORKER)) {
					...
					ply.sendMessage("Imbued this "+itemname+" with "+xpeditor.xpLookup[levels]+"xp (level 0->"+levels+")!");
				}
			}
		}
	}
